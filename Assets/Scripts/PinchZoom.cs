...
public class PinchZoom : MonoBehaviour
{
    ...
    float maximumOrthoSize;
    float maximumFOVSize;

    void Start()
    {

        maximumOrthoSize = GetComponent<Camera>().orthographicSize;
        maximumFOVSize = GetComponent<Camera>().fieldOfView;

        Debug.Log(maximumOrthoSize);
        Debug.Log(maximumFOVSize);
    ...
    void Update()
    {
        ...
        if (Input.touchCount == 2)
        {
            ...
            if (GetComponent<Camera>().orthographic)
            {
                ...
                if ((GetComponent<Camera>().orthographicSize + deltaMagnitudeDiff * orthoZoomSpeed) <= maximumOrthoSize)
                {
            ...
            else
            {
                ...
                if ((GetComponent<Camera>().fieldOfView + deltaMagnitudeDiff * perspectiveZoomSpeed) <= maximumFOVSize)
                {
